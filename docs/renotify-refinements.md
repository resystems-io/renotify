...
| 2026-03-27 | A-07 | Draft | Notification ID format specified: `ntf_` + 16 Crockford Base32 chars (80 bits from UUIDv7 truncated). Added to identifier summary table in naming analysis. Updated all payload schema exemplars with correct-length IDs. |
...
| 2026-03-28 | A-14 | Draft | Added `offset` field to `HistoryQueryRequest` for offset-based pagination. Updated SQLite history query with `OFFSET :offset`. Added `--offset` CLI flag to `renotify history`. |
